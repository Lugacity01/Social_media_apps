const HttpError = require("../models/errorModel");
const CommentModel = require('../models/commentModel')
const PostModel = require("../models/postModels");
const UserModel = require("../models/userModel");




// ==================== CREATE COMMENT
// POST : api/comments/:postId and is a PROTECTED ROUTE

const createComment = async (req, res, next) => {
  try {
    const {postId} = req.params;
    const {comment} = req.body;
    if(!comment){
        return next(new HttpError("Please write a comment", 422))
    } 

    // Get comment creator from db
    const commentCreator = await UserModel.findById(req.user.id)
    const newComment = await CommentModel.create({creator: {creatorId: req.user.id,
        creatorName: commentCreator?.fullName, creatorPhoto: commentCreator?.
        profilePhoto}, comment, postId})
        await PostModel.findByIdAndUpdate(postId, {$push:  {comments: newComment?._id}}, 
        {new: true})

        res.json(newComment)
  } catch (error) {
    return next(new HttpError);
  }
};


// ==================== GET POST COMMENT
// GET : api/comments/:postId and is a PROTECTED ROUTE

const getPostComment = async (req, res, next) => {
    try {
        const {postId} = req.params;
        const comments = await PostModel.findById(postId).populate({path: "comments", options: {sort: {createdAt: -1}}})
        res.json(comments)
    } catch (error) {
      return next(new HttpError);
    }
  };


  // ==================== DELETE COMMENT
// DELETE : api/comments/:commentId and is a PROTECTED ROUTE

const deleteComment = async (req, res, next) => {
    try {
      const {commentId} = req.params;

    //   Get the comment from db
    const comment = await CommentModel.findById(commentId);
    const commentCreator = await UserModel.findById(comment?.creator?.creatorId)

    // Check if the creator is the oner performing the deletion
     if(commentCreator?._id != req.user.id){
        return next(new HttpError("Unauthorized actions.", 403))
     }

    //  Remove comment id from post comment array
    await PostModel.findByIdAndUpdate(comment?.postId, {$pull: {comments: commentId}})
    const deletedComment = await CommentModel.findByIdAndDelete(commentId)
    res.json(deletedComment)
    } catch (error) {
      return next(new HttpError);
    }
  };

  module.exports = {createComment, getPostComment, deleteComment}